...
export default function TransaksiRecent({ transaksi = [] }) {
  ...
  if (recent.length === 0) {
    return (
      <div className="bg-white border border-border rounded-card p-6 text-center text-stone text-sm">
          ...
          <li key={t.id} className="flex items-center gap-3 px-4 py-3">
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                t.tipe === 'pemasukan' ? 'bg-[#E1F5EE]' : 'bg-[#FCEBEB]'
              }`}
            >
              {t.tipe === 'pemasukan' ? (
            ...
            <p
              className={`text-sm font-mono font-medium flex-shrink-0 ${
                t.tipe === 'pemasukan' ? 'text-success' : 'text-danger'
              }`}
            >
              {t.tipe === 'pemasukan' ? '+' : '-'}
